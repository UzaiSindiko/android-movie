...
export default function Movie({ navigation }) {
    ...
    const { loading, error, data } = useQuery(GET_TVSHOWS)
    const search = useQuery(SEARCH_GET_TVSHOWS, { variables: { q } } )
    ...
    if(!q){
        return (
            <View  style={{ flex: 1, alignItems: 'center', justifyContent: 'flex-start', marginBottom: 10 }}>
                <Text style={{ color: '#232323', fontSize: 40, marginTop: '10%', alignSelf: 'flex-start' }} > TV Show </Text>
                         ...
                         <ScrollView  horizontal={true} /* CARD CONTAINER*/  style={{ flexDirection: 'row', height: 200 }} >
                            {
                                ...
                                data.tvshows.map(v => (
                                    <View key={ v._id } /* CARD */  style={{ width: 100, height: 150, margin: 10}}>
                                        <TouchableOpacity onPress={() => navigation.navigate('TVShowDetail', {id: v._id})} >
                                            <Image 
                         ...
                         <ScrollView  horizontal={true} /* CARD CONTAINER*/  style={{ flexDirection: 'row', height: 110 }} >
                            {
                                ...
                                <View /* CARD */ key={ v._id }  style={{ width: 150, height: 100, margin: 10}}>
                                    <TouchableOpacity onPress={() => navigation.navigate('TrailerTvShow', {id: v._id})} >
                                        <Image 
                                            style={{width: '100%', height: '100%', borderRadius: 10}}
                                            source={{uri: v.poster_path }}
                                        />
                ...
                <ScrollView style={{ width: screenWidth, height: '80%'}}>
                   {
                       ...
                       <ActivityIndicator size="large" color="#0000ff" /> 
                       :
                        search.data.searchTVShow.map(v => (
                        <TouchableOpacity key={ v._id } onPress={() => navigation.navigate('TVShowDetail', {id: v._id})} >
